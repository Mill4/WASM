...
int SearchNumberOfAsFromPokemons(std::list<std::string> pokemons)
{
    int counter = 0;
    for (auto p : pokemons)
    {
...
void iterateSearchFunction(int iterations, std::list<std::string> pokemons)
{
    auto iteration_timer = std::chrono::high_resolution_clock::now();
    for (int i = 0; i < iterations; i++)
    {
        int res = SearchNumberOfAsFromPokemons(pokemons);
    }
    auto finish_iteration = std::chrono::high_resolution_clock::now();

    std::chrono::duration<double> elapsed = finish_iteration - iteration_timer;
    std::cout << "Time elapsed in " << iterations << " iterations: " << elapsed.count() << std::endl;
}

// C++ -  ijk (order of the loops)

void ijk(size_t length)
{
    unsigned matrixA[1][1];
    unsigned matrixB[1][1];
    unsigned matrixC[1][1];
...
int main()
{
    ...
    //iterateSearchFunction(100, pokemons);
    //iterateSearchFunction(10000, pokemons);
    //iterateSearchFunction(100000, pokemons);
    //iterateSearchFunction(500000, pokemons);

    size_t initial_size = 64;
    while (initial_size < 1025)
    {

        /* unsigned matrixA[initial_size][initial_size];
        unsigned matrixB[initial_size];
        unsigned matrixC[initial_size]; */

        auto ijk_start = std::chrono::high_resolution_clock::now();
        ijk(initial_size);
        auto ijk_end = std::chrono::high_resolution_clock::now();

        std::chrono::duration<double> ijk_elapsed = ijk_end - ijk_start;

        auto milliseconds = ijk_elapsed.count() * 1000.0;

        std::cout << "Time elapsed in array size " << initial_size << " : " << milliseconds << std::endl;
        // take time

        initial_size = initial_size * 2;
    }

    //ijk(matrixA, matrixB, matrixC, 32);
...
/*
std::list<std::string> pokemons{"Bulbasaur", "Ivysaur", "Venusaur", "Charmander",
                                    ...
                                    "Parasect", "Venonat", "Venomoth", "Diglett", "Meowth",
                                    "Persian", "Psyduck", "Golduck", "Mankey", "Primeape",
                                    "Growlithe", "Arcanine", "Poliwag", "Poliwhirl", "Poliwrath",
                                    "Abra", "Kadabra", "Machop", "Machoke", "Machamp", "Bellsprout",
